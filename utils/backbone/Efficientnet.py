...
class MBConvBlock(nn.Module):
    ...
    def __init__(self, block_args, global_params):
        ...
        if self._block_args.expand_ratio != 1:
            ...
            # self._bn0 = nn.GroupNorm(oup, oup, eps=1e-05, affine=True)
        ...
        self._bn1 = nn.BatchNorm2d(
            num_features=oup, momentum=self._bn_mom, eps=self._bn_eps)
        # self._bn1 = nn.GroupNorm(oup, oup, eps=1e-05, affine=True)
        ...
        # self._bn2 = nn.GroupNorm(final_oup, final_oup, eps=1e-05, affine=True)
...
class EfficientNet(nn.Module):
    ...
    def __init__(self, blocks_args=None, global_params=None, name=None):
        ...
        # self._bn0 = nn.GroupNorm(out_channels, out_channels, eps=1e-05, affine=True)
        ...
        self._bn1 = nn.BatchNorm2d(
            num_features=out_channels, momentum=bn_mom, eps=bn_eps)
        # self._bn1 = nn.GroupNorm(out_channels, out_channels, eps=1e-05, affine=True)
    ...
    @classmethod
    def from_name(cls, model_name, override_params=None, *args, **kwargs):
        cls._check_model_name_is_valid(model_name)
        blocks_args, global_params = get_model_params(
            model_name, override_params)
        return cls(blocks_args, global_params, name=model_name.split('-')[-1], *args, **kwargs)

    @classmethod
    def from_pretrained(cls, model_name, num_classes=1000, *args, **kwargs):
        kwargs['num_classes'] = num_classes
        model = cls.from_name(model_name, override_params=kwargs)
...
class Efficientnet(nn.Module):
    def __init__(self,pretrained,name,**k):
        super(Efficientnet,self).__init__()
        if pretrained:
            self.net = EfficientNet.from_pretrained(name,**k)
        else:
            self.net = EfficientNet.from_name(name,override_params=k)
